        ...
        <form action="{{ route('admin.umkm.produk.update', $produk->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div x-data="{ previewUrl: '' }">
                ...
                <div class="mb-3">
                    ...
                    <template x-if="!previewUrl">
                        @if($produk->gambar)
                            <img src="{{ asset('storage/' . $produk->gambar) }}" class="w-32 h-32 object-cover rounded-xl shadow-sm border border-gray-200">
                        @else
                            <div class="w-32 h-32 bg-gray-100 rounded-xl flex items-center justify-center border border-gray-200">
                                <span class="text-xs text-gray-400">Belum ada foto</span>
                            </div>
                        @endif
                    </template>
                </div>
                <input @change="if($event.target.files.length) previewUrl = URL.createObjectURL($event.target.files[0])" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-xl cursor-pointer bg-gray-50 focus:outline-none p-2.5" id="gambar" name="gambar" type="file" accept="image/*">
