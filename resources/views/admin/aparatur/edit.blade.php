            ...
            <div>
                ...
                <select name="jabatan" id="jabatan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-primary focus:border-primary block w-full p-2.5" required>
                    ...
                    @foreach($jabatanList as $jbt)
                        <option value="{{ $jbt }}" {{ old('jabatan', $aparatur->jabatan) == $jbt ? 'selected' : '' }}>{{ $jbt }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <div class="mb-3">
                    @if($aparatur->foto_url)
                        <img id="image_preview" src="{{ asset('storage/' . $aparatur->foto_url) }}" class="max-w-48 max-h-64 w-auto h-auto object-contain rounded-lg shadow-sm border border-gray-200" onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($aparatur->nama) }}&background=E5E7EB&color=374151&size=200'">
                    @else
                        <img id="image_preview" src="https://ui-avatars.com/api/?name={{ urlencode($aparatur->nama) }}&background=E5E7EB&color=374151&size=200" class="max-w-48 max-h-64 w-auto h-auto object-contain rounded-lg shadow-sm border border-gray-200">
                    @endif
                </div>
                <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-xl cursor-pointer bg-gray-50 focus:outline-none p-2.5" id="foto_url" name="foto_url" type="file" accept="image/*" onchange="previewImage(this)">
                <p class="mt-1 text-xs text-gray-500">Biarkan kosong jika tidak ingin mengubah foto. Format: JPG, PNG. Maksimal 5MB.</p>
